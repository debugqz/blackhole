...
![Audience](https://img.shields.io/badge/audience-contributors%20%26%20AI%20agents-blueviolet.svg)
![Scope](https://img.shields.io/badge/scope-non--negotiables%20%2B%20repo%20map-lightgrey.svg)
...
([English: docs/SPEC.en.md](docs/SPEC.en.md)).
...
and "what could go wrong here" questions are already answered there. For a
visual, feature-complete tour aimed at humans (badges, diagrams, status
tables), see **[README.md](README.md)** — this document is the denser,
contributor/agent-facing counterpart: non-negotiables plus a per-path map of
the repo, kept current as the code changes.

## Table of contents

- [Non-negotiables](#non-negotiables-do-not-casually-change)
- [Stack decisions](#stack-decisions-made-at-scaffold-time-not-fixed-in-specmd-v01)
- [Repo layout](#repo-layout)
  - [`daemon/`](#daemon)
  - [`crates/bh-crypto`](#cratesbh-crypto)
  - [`crates/bh-network`](#cratesbh-network)
  - [`crates/bh-storage`](#cratesbh-storage)
  - [`crates/bh-files`](#cratesbh-files)
  - [`crates/bh-api`](#cratesbh-api)
  - [`crates/bh-calls`](#cratesbh-calls)
  - [`crates/bh-push-relay`](#cratesbh-push-relay)
  - [`client/desktop`](#clientdesktop)
  - [`docs/SPEC.md`](#docsspecmd)
  - [`docs/THREAT_MODEL.md`](#docsthreat_modelmd)
  - [`infra/`](#infra)
- [Testing & CI status](#testing--ci-status)
...
Who depends on whom, at a glance — the daemon owns every library crate;
the client only ever talks to the daemon, never to a library crate or the
network directly:

```mermaid
graph TB
    Client["client/desktop<br/>Tauri UI"] -->|"HTTP, loopback only"| API["bh-api<br/>localhost RPC surface"]
    Daemon["daemon (bh-daemon binary)"] --> API
    Daemon --> Crypto["bh-crypto"]
    Daemon --> Storage["bh-storage"]
    Daemon --> Files["bh-files"]
    Daemon --> Calls["bh-calls"]
    Daemon --> Network["bh-network"]
    API --> Crypto
    API --> Storage
    API --> Files
    API --> Calls

    Daemon -.->|"opt-in, best-effort HTTP call<br/>after a real mailbox push"| PushRelay["bh-push-relay<br/>separate internet-facing binary"]

    Infra["infra/<br/>deploy artifacts, not app code"] -.->|"wraps/deploys"| Daemon
    Infra -.->|"wraps/deploys"| PushRelay

    style PushRelay fill:#1a1a2e,stroke:#666,stroke-dasharray: 4 4
    style Infra fill:#1a1a2e,stroke:#666,stroke-dasharray: 4 4
```

### `daemon/`

Rust binary; owns the SQLCipher database and platform
...
identity + seed phrase, passkeys/TOTP, X3DH + Double
...
libp2p transport + Kademlia DHT, onion routing,
...
SQLCipher-backed data model (contacts,
...
content-addressed file chunking, per-chunk E2EE,
...
localhost RPC surface between daemon and UI clients.
...
voice/video calls: real WebRTC transport (ICE/DTLS/
...
a small, separate, internet-facing binary (not
...
Tauri desktop client. Real product UI (monochrome
...
full spec, source of truth for architecture decisions.
...
per-subsystem STRIDE analysis grounded in the
actual implementation, plus a ranked list of known open risks.

### `infra/`

deploy artifacts (not application code) for the three real
